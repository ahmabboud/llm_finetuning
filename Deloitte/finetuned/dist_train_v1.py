...
def main() -> None:
    ...
    model_path = "/fs01/model-weights/Meta-Llama-3-8B-Instruct"
    # model_path = "/model-weights/Meta-Llama-3-8B"
    dataset_path = "/h/ws_vbhagat/llm_finetuning/Deloitte/finetuned/Vijay_Dist_Training/train_dataset.csv"
    batch_size = 2  # use batches of powers of 2
    epochs = 1
    ...
    if dist_type == "FSDP":
        with FSDP.state_dict_type(
            ...
            if rank == 0:
                torch.save(model_state, "finetuned_model_weights.pt")
    else:
        model_state = model.state_dict()
        if rank == 0:
            torch.save(model_state, "finetuned_model_weights.pt")
